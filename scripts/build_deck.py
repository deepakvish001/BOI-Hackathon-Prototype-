...
def build(m: dict) -> Presentation:
    ...
    for mem in TEAM:
        ...
        x += Inches(2.75)
    tb = text(s, Inches(0.9), Inches(6.98), Inches(11.5), Inches(0.35),
              "make setup && make all && make serve      ·      ",
              size=11, color=FAINT, font=MONO)
    # The repository is a live link, not decoration: a reviewer reading the deck
    # on screen should be one click from the code.
    link = tb.text_frame.paragraphs[0].add_run()
    link.text = REPO_LABEL
...
def main() -> int:
    ...
    # python-pptx otherwise leaves its own author and a library credit in the
    # file properties, which is not what a submitted deck should say about
    # itself.
    cp = prs.core_properties
    cp.title = "BODHI MULE HUNTER AI"
